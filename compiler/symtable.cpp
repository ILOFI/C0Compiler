...
int insertTable(string name, symkind kind, symtype type, int value, int address, int len)
{
    ...
    if (kind == FUNCKD)     //函数类符号，需要检查所有分程序，是否有重复定义的函数名
    {
        for (int i = 1; i < symbolTable.totSub; i++)
            if (symbolTable.item[symbolTable.subpnt[i]].name == name)
    ...
    {   
        ...
        for (int i = symbolTable.subpnt[symbolTable.totSub-1]; i < symbolTable.curpnt; i++)
            if (symbolTable.item[i].name == name)
            {
                errmain();
                return -1;
            }
    ...
    symbolTable.item[symbolTable.curpnt].name = name;
...
{
    //函数声明后返填符号表的参数个数信息
    int pnt = symbolTable.subpnt[symbolTable.totSub - 1];
...
int searchTable(string name, bool findfunc)
{
    ...
    {
        //查找变量、常量和参数
        //当前的分程序序号为totSub-1，首先在当前层找
    
        for (int i = symbolTable.subpnt[symbolTable.totSub-1]+1; i < symbolTable.curpnt; i++)
        {
            if (symbolTable.item[i].kind == FUNCKD) //找到下一层函数名，越界了
                break;
            if (symbolTable.item[i].name == name)   //成功找到
                return i;
        }
